...
async function dexGet(path) {
    ...
    if (!r.ok)
        throw new Error(`DexScreener error ${r.status}: ${r.statusText}`);
    return r.json();
}
const TOOLS = [
    {
...
async function handleTool(name, args) {
    switch (name) {
        ...
        case 'dx_trending_bsc': {
            ...
            const pairs = (data.pairs || []).filter((p) => (p.liquidity?.usd || 0) >= minLiq &&
                (p.volume?.h1 || 0) >= minVol1h);
            pairs.sort((a, b) => (b.priceChange?.h1 || 0) - (a.priceChange?.h1 || 0));
        ...
        case 'dx_pair': {
            ...
            const pairs = data.pairs || [];
            if (!pairs.length)
                return JSON.stringify({ error: 'Pair not found' });
...
function createServer() {
    const server = new Server({ name: 'dexscreener-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    app.post('/mcp', async (req, res) => {
        ...
        try {
            ...
            if (sessionId && transports[sessionId]) {
                ...
                transport.onclose = () => { const sid = transport.sessionId; if (sid)
